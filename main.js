...
const loadAssets = async (names) => {
    ...
    const loadModels = n => {
        ...
        const location = [`./models/${n}.ply`, ...models.map(m => `./models/${n}_${m}.ply`)]
    ...
    names.forEach(async n => {
        const models = await loadModels(n)
        assets[n] = models
    })
    return assets
}
...
const init = async (assets) => {
    ...
    const changeModel = () => {
        if (gui) gui.destroy()
        if (curModel) {
            assets[curModel].forEach(item => { item.visible = false })
        }
